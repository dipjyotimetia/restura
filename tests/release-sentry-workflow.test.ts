...
describe('release workflow Sentry guardrails', () => {
  ...
  it('prepares stable releases through an App-authenticated post-CI merge and never pushes main directly', () => {
    ...
    expect(workflow).toContain("github.event.workflow_run.head_branch == 'release/prepare'");
    expect(workflow).toContain('Merge validated release candidate');
