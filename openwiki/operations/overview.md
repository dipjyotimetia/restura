...
type: Operations
title: Operations
description: 'How Restura is built, run, tested, packaged, and deployed, including development setup, targets, CLI, CI/CD, telemetry, and troubleshooting.'
tags: [build, deploy, ci-cd, telemetry, development]
---
...
```mermaid
flowchart TD
    START([npm run validate]) --> TC[type-check:all]
    TC --> LINT[lint]
    LINT --> FMT[format:check]
    FMT --> OC[verify:opencollection-types]
    OC --> CAP[capabilities:check]
    CAP --> ARCH[architecture:check]
    ARCH --> TEST[root tests + CLI/VS Code tests with coverage]
    TEST --> BUILD[production builds web worker CLI extension electron compile]
    BUILD --> SIZE[bundle-size limits]
    SIZE --> END([done])
```
_`validate` runs static checks, generated-file drift checks, tests, and production builds sequentially; any failure blocks the gate._
