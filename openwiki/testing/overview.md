...
- **Desktop Kafka producer**: `src/features/kafka/lib/__tests__/kafkaProducerValidation.test.ts`, `electron/main/__tests__/{kafka-handler,kafka-serde,kafka-validators}.test.ts`, and `src/features/kafka/lib/__tests__/kafkaManager.electron.test.ts` cover client-side payload/header/schema validation, strict Base64 wire decoding and byte-preserving consume display, IPC schema validation, and manager routing. These tests protect the desktop behavior described in the [Kafka / MQTT section of Protocol features](../features/protocols.md).
- **Console evidence**: `src/store/__tests__/useConsoleStore.test.ts` covers entry/frame sanitization, native draft reconstruction, persistence trimming, and pin/preserve/capture controls. `src/lib/shared/__tests__/console-export.test.ts` covers HAR/NDJSON/cURL export correctness and credential-free output. `src/features/collections/lib/__tests__/collectionRunner.test.ts` covers response evidence retention and redaction. See [Console evidence & safety](../features/console-evidence.md).
