...
```mermaid
flowchart TD
    FILE[external collection file] -->|detect format| DISPATCH[src/features/collections/lib/importers.ts]
    DISPATCH -->|Postman| PM[postman transformer]
    ...
    EDIT --> STRIP[strip ancestor _oc bags]
    STRIP --> EXPORT[re-export from live model]
```
_Imports are dispatched by detected format, normalized to the Restura model, and persisted. OpenCollection subtrees keep a verbatim `_oc` cache until edited; re-export then rebuilds from the live model._
