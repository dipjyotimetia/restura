...
type: Workflows
title: Workflows
description: 'Open Workflow Specification-backed orchestration: data model, supported profile, execution paths, editor, and CLI support.'
tags: [workflows, ows, orchestration, editor, cli]
---
...
erDiagram
    ...
    OWS["workflow.ows.json"] {
        object sdkNormalizedDocument
    }
    BINDINGS["bindings.restura.json"] {
        map taskPathToSavedRequest
    }
    LAYOUT["layout.restura.json"] {
        object nonSemanticLayout
    }
    ...
    WORKFLOW ||--|| OWS : contains
    WORKFLOW ||--|| BINDINGS : contains
    WORKFLOW ||--|| LAYOUT : contains
    BINDINGS }|--|| REQUEST : "references by id"
```
_The workflow record wraps three distinct artifacts: the OWS document, the Restura-specific bindings, and the non-semantic editor layout. Bindings point to saved requests by ID._
...
```mermaid
sequenceDiagram
    participant BUILD as WorkflowBuilder.tsx
    participant EXEC as shared/ows/executor.ts
    participant HOOK as useOwsWorkflowExecution.ts
    participant REG as ProtocolRegistry
    participant CLI as cli/src/runner/owsWorkspaceRunner.ts

    BUILD->>EXEC: run workflow OWS document
    EXEC->>EXEC: validate against safe profile
    loop for each task
        EXEC->>HOOK: dispatch task path binding method signal
        HOOK->>REG: resolve saved HTTP request
        REG-->>HOOK: response / stream result
        HOOK-->>EXEC: task output
    end
    EXEC-->>BUILD: execution result

    alt CLI run
        CLI->>EXEC: run workspace artifacts
        EXEC->>CLI: dispatch task
        CLI->>CLI: HTTP runner via undiciFetcher
    end
```
_The executor walks the validated OWS document and dispatches each HTTP/GraphQL task through the normal protocol registry or CLI HTTP runner. Raw OWS call configuration never reaches a transport._
