...
<Aside type="caution" title="gRPC wire format">
  The CLI uses **Connect-over-HTTP**, not gRPC-over-HTTP/2 binary framing. If your server only
  speaks classic gRPC, requests come back `UNKNOWN`.
</Aside>
...
<Aside title="Canonical source">
  This page mirrors
  [`cli/README.md`](https://github.com/dipjyotimetia/restura/blob/main/cli/README.md). The README is
  the source of truth for flag changes.
</Aside>
