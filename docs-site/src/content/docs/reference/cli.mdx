...
| `--ca <file>` | | PEM CA bundle to trust (private CA). |
| `--client-cert <file>` | | PEM client certificate for mutual TLS. |
| `--client-key <file>` | | PEM client private key for mutual TLS. |
| `--cert-passphrase <value>` | | Passphrase for an encrypted client key. |
| `--proxy <url>` | | HTTP(S) proxy URL (overrides `HTTP_PROXY`; composes with TLS options). |
