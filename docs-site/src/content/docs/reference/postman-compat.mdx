...
Restura's primary namespace is `rs.*`; `pm.*` is the compatibility alias. Imported Postman scripts are normalized to `rs.*` and rewritten back to `pm.*` on export. See [Scripts](/guides/scripts/).
</Aside>
