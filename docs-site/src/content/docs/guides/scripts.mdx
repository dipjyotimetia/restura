...
Scripts use the native **`rs.*`** namespace. For drop-in Postman compatibility, **`pm`** is a live alias of the _same_ object — `rs.expect(...)` and `pm.expect(...)` are identical, so existing Postman scripts run unchanged.
...
<Aside type="tip" title="Editor IntelliSense">
  The script editors are Monaco with full type definitions registered for the API — you get
  autocomplete, member completion (`rs.`, `rs.response.`), hover docs, and signature help as you
  type.
</Aside>
...
<Aside title="Scripting vs. workflows">
  For multi-step orchestration with branching, retries, and parallelism, a
  [workflow](/guides/workflows/) is usually clearer than chaining `rs.sendRequest` calls in one
  script.
</Aside>
