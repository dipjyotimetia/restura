...
<Aside type="note">
  Shortcuts are aware of focus. Editor shortcuts only trigger inside an editor; navigation shortcuts
  trigger when no input is focused — so `Cmd+T` opens a new tab, never inserts a `T` into your body.
</Aside>
