...
The tool surface is intentionally read-only and metadata-shaped. Your agent can ask _what_ requests exist; it can't trigger sends or modify saved state.
...
<Aside type="caution">
  MCP server mode is opt-in. It's off by default. When on, the desktop app only listens on
  `127.0.0.1` — never on a public interface.
</Aside>

## Related

- [MCP protocol](/protocols/mcp/) — Restura _as a client_ of other MCP servers.
