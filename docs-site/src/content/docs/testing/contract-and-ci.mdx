...
`npm run validate` is the canonical local and required-CI gate. It runs three explicit stages:
...
3. `validate:build` — web/Worker and self-host Node/Docker builds, CLI and extension bundles, Electron main/preload compilation with runtime-import verification, and renderer bundle-size budgets.
