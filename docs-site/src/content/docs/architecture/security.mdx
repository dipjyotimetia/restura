...
- Default: **no app-layer encryption at rest** (`PlaintextKeyProvider`) — IndexedDB is protected only by the browser's same-origin policy. This replaced an earlier ephemeral-key mode that corrupted data on every reload (the random key never survived the session).
- Future: an opt-in passphrase prompt that derives a stable session key via PBKDF2 (`WebSessionPassphraseProvider`).
...
- `nodeIntegration: false`, `contextIsolation: true`, `sandbox: true`, `webSecurity: true` on every window; `<webview>` attachment and all production navigation are denied.
- **Every IPC handler validates its sender frame** (`assertTrustedSender`) — only the `file://` app shell (or the dev server) can drive the main process.
- **Deny-by-default permissions** (`electron/main/permission-policy.ts`): the renderer may only request sanitized clipboard writes (the copy buttons). Camera, microphone, geolocation, MIDI, and the Notification web API are refused, so a rendered-response XSS can't silently obtain a device permission.
...
- [Security design](/architecture/security-design) — threat model, trust boundaries, secret-storage & code-signing design.
