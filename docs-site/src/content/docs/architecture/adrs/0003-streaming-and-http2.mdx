...
The gRPC-transport decision below (§3 — Connect-Web + `grpc-js`) was replaced by [ADR 0022](/architecture/adrs/0022-grpc-connectrpc-transport/): gRPC now runs on ConnectRPC (`connect-node` on desktop, Connect protocol on web). The HTTP-streaming and response-viewer decisions on this page still stand.
</Aside>
