...
- The session-ephemeral default for web means data does NOT persist across reloads. A passphrase-prompt UI (deferred to a follow-up) restores cross-session persistence with PBKDF2-derived keys.
  - **Superseded:** the web default later changed from `EphemeralKeyProvider` to `PlaintextKeyProvider` (no at-rest envelope; same-origin only) because the ephemeral key corrupted data on every reload. The opt-in `WebSessionPassphraseProvider` (PBKDF2) remains the cross-session path.
