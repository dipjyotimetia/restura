---
title: ADR 0017 — Runtime platform detection
description: isElectron() picks IPC vs HTTP transport from one bundle.
sidebar:
  order: 17
---
...
- Renderer code can be tempted to sprinkle `isElectron()` checks widely; the discipline is to confine them to transport selection, not feature logic — capability differences belong in `capabilities.ts` ([ADR 0012](/architecture/adrs/0012-capability-matrix-source-of-truth/)).

## References

- Source: [`docs/adr/0017-runtime-platform-detection.md`](https://github.com/dipjyotimetia/restura/blob/main/docs/adr/0017-runtime-platform-detection.md)
- Related: [ADR 0001](/architecture/adrs/0001-shared-protocol-layer/), [ADR 0012](/architecture/adrs/0012-capability-matrix-source-of-truth/).
