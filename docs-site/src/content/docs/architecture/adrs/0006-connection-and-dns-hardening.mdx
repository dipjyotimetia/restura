...
The pre-flight resolves once; if the DNS record is swapped with TTL=0 between the pre-flight and the C++ binding's own internal resolve, the connect lands on the post-swap address. The 2026-05-27 update (below) closes this for ws / sse / grpc.
</Aside>
...
- Tests: [Security test suite](/testing/security/) — SSRF, header policy, and redirect handling regressions.
