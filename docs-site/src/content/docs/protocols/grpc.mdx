---
title: gRPC
description: Unary and streaming RPCs over the Connect protocol (web) and native gRPC (desktop) — including server reflection, .proto upload, and W3C traceparent injection.
---

import { Badge } from '@astrojs/starlight/components';

Restura speaks the **Connect protocol** on the web target and **native gRPC** over HTTP/2 (with automatic Connect-protocol fallback) on desktop — no local toolchain either way. JSON encoding is the default; binary protobuf is supported where the server allows.
...
- [Workflows](/guides/workflows/) — OWS orchestration currently supports bound HTTP calls only; gRPC workflow calls are rejected until a bounded dispatcher exists.
