...
- **Desktop SDK compatibility** — desktop connections use the current MCP v2 client with automatic modern/legacy protocol negotiation. If a server rejects that protocol or transport handshake, Restura retries once through the retained v1 SDK; authentication, TLS, network, timeout, and security-policy failures are never retried.
