...
- `docs/adr/` — numbered canonical ADRs (`NNNN-title.md`).
...
| **Architectural decision** (new transport, security boundary, persistence, sandbox, build system, IPC pattern) | new `docs/adr/NNNN-*.md`; matching `docs-site/.../architecture/adrs/NNNN-*.mdx`; timeline + `LinkCard` in `adrs.mdx`; sidebar in `docs-site/astro.config.mjs` |
...
| **npm scripts / validation / CI / release**                                                                    | `AGENTS.md`, `CLAUDE.md`, `README.md`, `docs/CI_CD.md`, `openwiki/quickstart.md`, OpenWiki testing/operations, docs-site development setup + contract/CI pages, and `.codex/README.md` for Codex lifecycle changes |
...
When you add `docs/adr/NNNN-*.md`: use the next number, date it, add the matching
docs-site ADR page, then update the timeline, `LinkCard` grid, and Starlight
sidebar.
