...
describe('AiChatRequestSchema — API-key requirement for cloud providers', () => {
  ...
  it.each([
    ...
    'anthropic',
    'openrouter',
  ])('rejects a cloud provider (%s) WITHOUT an API key handle', (provider) => {
    const r = AiChatRequestSchema.safeParse(
...
describe('AiLabCompleteSchema / AiLabStreamSchema — API-key requirement', () => {
  it.each([
    ...
    'openrouter',
    'huggingface',
  ])('rejects a cloud provider (%s) complete WITHOUT an API key handle', (provider) => {
  ...
  it.each([
    ...
    'openrouter',
    'huggingface',
  ])('accepts a cloud provider (%s) complete WITH an API key handle', (provider) => {
  ...
  it.each([
    'ollama',
    'openai-compatible',
  ])('accepts a keyless local provider (%s) for complete', (provider) => {
    const r = AiLabCompleteSchema.safeParse(
      completeSpec({
        provider,
        ...(provider === 'openai-compatible' ? { baseUrlOverride: 'http://localhost:11434' } : {}),
      })
    );
    expect(r.success).toBe(true);
  });
...
describe('AiLabDiscoverSchema — API-key requirement for discovery', () => {
  it.each([
    ...
    'anthropic',
    'huggingface',
  ])('rejects a key-required cloud provider (%s) discovery WITHOUT any key', (provider) => {
  ...
  it.each([
    ...
    'anthropic',
    'huggingface',
  ])('accepts a key-required cloud provider (%s) discovery WITH a plaintext apiKey', (provider) => {
    const r = AiLabDiscoverSchema.safeParse({
      provider,
      baseUrl: 'https://api.openai.com',
      apiKey: 'sk-test',
  ...
  it.each([
    ...
    'anthropic',
    'huggingface',
  ])('accepts a key-required cloud provider (%s) discovery WITH an apiKeyHandleId', (provider) => {
    const r = AiLabDiscoverSchema.safeParse({
      provider,
      baseUrl: 'https://api.openai.com',
      apiKeyHandleId: crypto.randomUUID(),
  ...
  it.each([
    'ollama',
    'openai-compatible',
  ])('accepts a keyless local provider (%s) discovery', (provider) => {
    const r = AiLabDiscoverSchema.safeParse({
      provider,
