...
describe('materializeExternalProtocolAuth', () => {
  it('fails closed before wire signing when a desktop external profile is unavailable', async () => {
    await expect(
      materializeExternalProtocolAuth({
        type: 'aws-signature',
        awsSignature: {
          accessKey: 'AKIA',
          secretKey: {
  ...
  it('also fails closed for bearer tokens before main applies an Authorization header', async () => {
    await expect(
      materializeExternalProtocolAuth({
        type: 'bearer',
        bearer: {
    ...
    ).rejects.toThrow('Azure Key Vault profile is unavailable.');
  });
});
