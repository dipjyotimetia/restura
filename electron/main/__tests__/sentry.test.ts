...
// exercise the captured beforeSend/beforeBreadcrumb directly.
...
describe('opt-in gate', () => {
  ...
  it('inits when enabled and beforeSend honours the runtime gate', () => {
    vi.stubEnv('SENTRY_DSN', 'https://examplePublicKey@o0.ingest.sentry.io/0');
    initSentry({ enabled: true });
    expect(initMock).toHaveBeenCalledTimes(1);

    const opts = initMock.mock.calls[0]![0] as { beforeSend: (e: unknown) => unknown };
