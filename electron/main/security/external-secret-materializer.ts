...
const FIELDS: Array<[keyof ProtocolAuthConfig, string[]]> = [
  ['awsSignature', ['secretKey']], ['oauth1', ['consumerSecret', 'accessToken', 'accessTokenSecret']], ['ntlm', ['password']], ['wsse', ['password']],
];
...
export async function materializeExternalProtocolAuth(
  ...
  for (const [block, fields] of FIELDS) {
    ...
    for (const field of fields) {
      ...
      if (isProtocolExternalSecret(value)) { copy[field] = await desktopExternalSecretResolver.resolve(value, { signal }); changed = true; }
    }
