...
export async function resolveSafeAddress(
  ...
  const port = parsed.port
    ...
    : (options.defaultPort ?? (parsed.protocol === 'https:' || parsed.protocol === 'wss:' ? 443 : 80));
...
export function createPinnedLookup(host: string, ip: string): any {
  ...
  return (
    ...
    const callback = (typeof optsOrCb === 'function' ? optsOrCb : maybeCb) as
      ...
      | undefined;
    const lookupOptions = (typeof optsOrCb === 'object' && optsOrCb !== null ? optsOrCb : {}) as dns.LookupOptions;
    if (!callback) return;
    if (hostname !== host) {
