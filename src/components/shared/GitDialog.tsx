...
import { GitBranch as GitBranchIcon, Check, RefreshCw, GitCommit as GitCommitIcon } from 'lucide-react';
...
export function GitDialog({ collectionName, directoryPath, open, onClose }: GitDialogProps) {
  const { status, branches, log, loading, error, refresh, commit, createBranch, checkout } =
                  ...
                  {changedFiles.map((f) => (
                    ...
                    >
                      ...
                      <span className="font-mono text-sp-11-5 text-sp-dim w-6 uppercase">
                        {(f.staged !== '.' && f.staged) || (f.unstaged !== '.' && f.unstaged) || '?'}
                      </span>
                      <span className="font-mono text-sp-12 text-sp-text truncate">{f.path}</span>
                    </label>
                  ))}
                </div>
                <textarea
