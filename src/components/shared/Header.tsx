...
export default function Header({
  ...
  if (!mounted) {
    ...
    return (
      <header className="relative z-50 flex h-14 items-center justify-between border-b border-border bg-background px-6 app-drag">
    ...
    <header className="relative z-50 flex h-14 items-center justify-between border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 px-4 shadow-sm transition-all app-drag">
      ...
      <TooltipProvider delayDuration={300}>
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1 mr-1 lg:mr-2 app-no-drag">
            <Select
              value={activeEnvironmentId || 'none'}
              onValueChange={(value) => setActiveEnvironment(value === 'none' ? null : value)}
            >
              <SelectTrigger className="h-7 lg:h-8 w-[120px] lg:w-[160px] border-transparent bg-muted/30 hover:bg-muted/50 focus:ring-0 text-[10px] lg:text-xs app-no-drag">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className="h-7 w-7 lg:h-8 lg:w-8 text-muted-foreground app-no-drag"
          ...
          {/* Actions Group */}
          <div className="flex items-center gap-0.5 lg:gap-1 app-no-drag">
            <CollectionRunner />
