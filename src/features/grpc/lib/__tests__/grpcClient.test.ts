...
`;
  ...
  describe('makeElectronGrpcRequest (non-Electron)', () => {
    it('should throw when not in Electron environment', async () => {
      const request: GrpcRequest = {
        id: 'req-1',
        name: 'Test',
        type: 'grpc',
      ...
      await expect(makeElectronGrpcRequest(request, '', 'test.proto', (t) => t)).rejects.toThrow(
        'Electron environment required'
      );
    });
  });

  describe('startElectronGrpcStream', () => {
    it('should throw when not in Electron environment for server-streaming', () => {
      const request: GrpcRequest = {
        id: 'req-stream',
        name: 'Stream Test',
        type: 'grpc',
      ...
      expect(() =>
        startElectronGrpcStream(request, '', 'test.proto', (t) => t, {
          onData: vi.fn(),
          onError: vi.fn(),
          onStatus: vi.fn(),
        })
      ).toThrow('Electron environment required');
    });

    it('should throw when not in Electron environment for client-streaming', () => {
      const request: GrpcRequest = {
        id: 'req-client-stream',
        name: 'Client Stream Test',
        type: 'grpc',
    ...
    it('should throw when not in Electron environment for bidirectional-streaming', () => {
      const request: GrpcRequest = {
        id: 'req-bidi',
        name: 'Bidi Stream Test',
        type: 'grpc',
    ...
    it('should use Electron IPC when api is available (mocked)', () => {
      const mockGrpc = {
        startStream: vi.fn(),
        sendMessage: vi.fn(),
        endStream: vi.fn(),
      ...
      // isElectron() checks window.electron?.isElectron === true
      // getElectronAPI() returns window.electron
      Object.defineProperty(window, 'electron', {
        value: { isElectron: true, grpc: mockGrpc },
        writable: true,
        configurable: true,
      });
      ...
      expect(mockGrpc.on).toHaveBeenCalledTimes(3);
      expect(control.sendMessage).toBeDefined();
      expect(control.endStream).toBeDefined();
      expect(control.cancelStream).toBeDefined();

      // Test sendMessage delegates to IPC
      control.sendMessage({ text: 'hello' });
      expect(mockGrpc.sendMessage).toHaveBeenCalledWith('req-mocked', { text: 'hello' });

      // Test endStream delegates to IPC
      control.endStream();
      expect(mockGrpc.endStream).toHaveBeenCalledWith('req-mocked');

      // Test cancelStream removes listeners
      control.cancelStream();
      expect(mockGrpc.cancelStream).toHaveBeenCalledWith('req-mocked');
      expect(mockGrpc.removeListener).toHaveBeenCalledTimes(3);
