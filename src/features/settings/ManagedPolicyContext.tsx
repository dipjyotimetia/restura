  ...
  return (
    ...
    >
      <p className="font-semibold">
        {invalid
          ? 'Managed network policy needs administrator attention'
          : 'Network and updates managed by your organization'}
      </p>
      <p className="mt-1 text-sp-muted">
        {invalid ? status.message : `${status.networkMode} network · ${status.updatesMode} updates`}
      </p>
    </div>
  );
}
