...
The **Agents** tab uses the backend-agnostic `shared/agent-lab/` core: versioned suites, open capability-driven provider adapters, typed traces, bounded tool loops, approvals, repeated-trial statistics, trajectory/outcome graders, MCP/sandbox extension contracts, and opt-in OTLP/OpenInference export. Desktop adapters live in `lib/agentRuntime.ts`/`agentTools.ts`; saved HTTP request tools reuse `executeRequest`, and non-read calls require approval. `restura agent eval` consumes the same suite/runner for headless CI. Keep the capability matrix honest: an adapter contract is not shipped support until its desktop/CLI resolver is wired end to end.
