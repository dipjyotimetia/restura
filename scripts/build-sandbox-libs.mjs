...
`
  ...
  if (!existsSync(cryptoShim)) {
    await writeFile(
      cryptoShim,
      `// Minimal crypto shim — uuid only needs randomFillSync / randomUUID;
// QuickJS has Math.random() (not cryptographically secure but acceptable
// inside a sandbox where the host already prevents network egress).
