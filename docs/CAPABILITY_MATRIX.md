...
| AI Lab agent suites (multi-step trials, trajectory/outcome evals) | ❌ | ✅ | Electron-only authoring and execution with one cancellable run lifecycle; versioned suites, task-aware grading, hard run-wide budgets, repeated-trial reliability, and sanitized bounded report persistence. Headless OpenAI Responses CI is available via `restura agent eval` |
