...
| AI Lab agents using MCP servers as tools | ❌ | ✅ | Desktop opens a fresh validated MCP session per run, applies the suite allowlist, and always requires approval because remote annotations do not grant local authority. CLI supports manifest-listed MCP tools only when CI explicitly asserts readOnly:true |
