...
| Visual system         | Reuse existing Spatial Depth tokens (`sp-floater`, `sp-floater-lg`, `--sp-accent`, OKLCH ladder) — no new token set                                                                              | CLAUDE.md/[[project_glass_design]] already establishes this system; extending it beats introducing a second one                                                            |
...
- **Sidebar** — ~240px fixed width, `sp-floater-lg` (frosted, vibrancy) on Electron, `sp-floater` (opaque) on web — same token, different computed backdrop per the tokens' existing light/dark + platform rules. Section rows: leading icon (lucide, consistent with icons used elsewhere in the app chrome) + label. Active row: `--sp-accent`-tinted background plus a left accent bar. Hover: existing `--sp-hover-bg`.
