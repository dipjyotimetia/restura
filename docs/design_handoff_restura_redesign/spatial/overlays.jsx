...
function CommandPalette({ p, open, onClose, onSelect }) {
  ...
  const items = React.useMemo(() => [
    { group: 'Requests', kind: 'req', method: 'POST',  name: 'Create order',         path: 'Restura API · Orders', go: 'create-order' },
    { group: 'Requests', kind: 'req', method: 'GET',   name: 'List orders',          path: 'Restura API · Orders', recent: true, go: 'list-orders' },
    { group: 'Requests', kind: 'req', method: 'PATCH', name: 'Update order status',  path: 'Restura API · Orders', go: 'update-order' },
    { group: 'Requests', kind: 'req', method: 'GET',   name: 'Get order by id',      path: 'Restura API · Orders', go: 'get-order' },
  ...
  React.useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (e.key === 'Enter')     { e.preventDefault(); onSelect && onSelect(items[idx]); onClose(); }
    };
  ...
  const filtered = q
    ? items.filter((it) => it.name.toLowerCase().includes(q.toLowerCase()) || (it.path && it.path.toLowerCase().includes(q.toLowerCase())))
    : items;
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        width: 640, maxHeight: 480, background: p.surfaceHi,
        borderRadius: 14, border: `1px solid ${p.lineStrong}`,
        ...
        <div style={{
          ...
          <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)}
            ...
            style={{
              ...
              fontFamily: 'Geist, sans-serif', fontSize: 14, color: p.text,
            }} />
          <Kbd p={p}>ESC</Kbd>
        </div>
            ...
            return (
              <React.Fragment key={g}>
                <div style={{
                  ...
                  color: p.textDim, textTransform: 'uppercase',
                }}>{g}</div>
                {inGroup.map((it) => {
                  const gIdx = filtered.indexOf(it);
                  const highlighted = gIdx === idx;
                  return (
                    <div key={it.name} onMouseEnter={() => setIdx(gIdx)}
                      onClick={() => { onSelect && onSelect(it); onClose(); }}
                      style={{
                      ...
                      {it.icon && <span style={{ color: p.textMuted, display: 'flex' }}>{SIcon[it.icon]({ size: 14 })}</span>}
                      <span style={{ color: p.text, fontWeight: 500, whiteSpace: 'nowrap' }}>
                        {it.name}
                      </span>
                      {it.path && <span style={{ color: p.textDim, fontSize: 11, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0 }}>{it.path}</span>}
                      {it.recent && <span style={{
                        fontSize: 9.5, color: p.textDim,
                        padding: '1px 6px', borderRadius: 4,
                        background: p.surfaceLo, letterSpacing: 0.5, fontWeight: 600,
                      }}>RECENT</span>}
                      <div style={{ flex: 1 }} />
        ...
        <div style={{
          ...
          display: 'flex', gap: 18,
          fontSize: 10.5, color: p.textDim,
        }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Kbd p={p}>↑↓</Kbd> navigate</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Kbd p={p}>↵</Kbd> select</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Kbd p={p}>⌘↵</Kbd> in new tab</span>
          <div style={{ flex: 1 }} />
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          height: 56, flexShrink: 0, padding: '0 22px',
          ...
          <div onClick={onClose} style={{
            ...
            color: p.textMuted, cursor: 'pointer', background: p.surfaceLo,
          }}><SIcon.close size={14} /></div>
        </div>
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: '220px 1fr', minHeight: 0 }}>
          {/* sections nav */}
          <div style={{ padding: 12, borderRight: `1px solid ${p.line}`, display: 'flex', flexDirection: 'column', gap: 2 }}>
            {sections.map((s) => (
              <div key={s.name} onClick={() => setSection(s.name)} style={{
                display: 'flex', alignItems: 'center', gap: 10,
                padding: '8px 10px', borderRadius: 8,
                fontSize: 12.5, fontWeight: s.name === section ? 600 : 500,
                color: s.name === section ? p.text : p.textMuted,
                background: s.name === section ? p.activeBg : 'transparent',
                cursor: 'pointer',
              }}>
                <span style={{ color: s.name === section ? p.accent : p.textMuted, display: 'flex' }}>{SIcon[s.icon]({ size: 14 })}</span>
                {s.name}
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: 28, overflow: 'auto' }}>
            {section === 'General' && (
              <React.Fragment>
                ...
                <ToggleField p={p} on={true} label="Auto-save to history"   hint="Every send is captured for replay" />
                <ToggleField p={p} on={true} label="Sync across devices"     hint="Encrypted with workspace key" />
                ...
                <ToggleField p={p} on={true}  label="Check for updates automatically" hint="Notify when a new version is available" />
                <FieldRow p={p} label="Release channel" hint="Beta gets new features ~2 weeks earlier">
            ...
            {section === 'Appearance' && (
              <React.Fragment>
                ...
                <FieldRow p={p} label="Mode" hint="Match system, or pin a fixed mode">
                  <Segmented p={p} value={theme} options={['light', 'dark']} onChange={onThemeChange} />
                </FieldRow>
                ...
                <FieldRow p={p} label="Font family" hint="Code editor & response viewer">
                  <Segmented p={p} value="JetBrains Mono" options={['JetBrains Mono', 'SF Mono', 'Berkeley Mono']} />
                </FieldRow>
            ...
            {section === 'Requests' && (
              <React.Fragment>
                ...
                <ToggleField p={p} on={true}  label="Follow redirects"        hint="Resolve 3xx chain up to 10 hops" />
                <ToggleField p={p} on={false} label="Verify SSL certificate" hint="Allow self-signed for dev environments" />
            ...
            {section === 'Proxy' && (
              <React.Fragment>
                <H1 p={p}>Proxy</H1>
                <ToggleField p={p} on={false} label="Use proxy server" hint="Route all requests through a proxy" />
              ...
              <React.Fragment>
                ...
                {[
                  ...
                  <div key={k} style={{
                    ...
                    padding: '11px 0', borderBottom: `1px solid ${p.line}`,
                  }}>
                    <span style={{ fontSize: 13, color: p.text }}>{k}</span>
                    <div style={{ display: 'flex', gap: 4 }}>
                      {v.split(' ').map((part, i) => <Kbd key={i} p={p}>{part}</Kbd>)}
                    </div>
                  </div>
                ))}
              </React.Fragment>
            ...
            {(section === 'Certificates' || section === 'Secrets' || section === 'About') && (
              <React.Fragment>
                ...
                <div style={{ color: p.textMuted, fontSize: 13, padding: '24px 0' }}>
                  {section === 'About'
                    ? <React.Fragment>Restura v1.4.2 · macOS · Electron 28<br/><br/>Made for people who poke at APIs all day.</React.Fragment>
...
const SectionLabel = ({ p, children }) => (
  <div style={{
    fontSize: 11, fontWeight: 700, letterSpacing: 0.7, color: p.textDim,
...
const FieldRow = ({ p, label, hint, children }) => (
  <div style={{
    ...
    padding: '12px 0', borderBottom: `1px solid ${p.line}`,
  }}>
    <div>
...
const ToggleField = ({ p, on, label, hint }) => (
  <FieldRow p={p} label={label} hint={hint}>
    <div style={{
      ...
      boxShadow: on ? `0 4px 10px ${p.accent}55` : 'none', cursor: 'pointer',
    }}>
      <div style={{
...
const Segmented = ({ p, options, value, onChange }) => (
  <div style={{ display: 'flex', padding: 2, borderRadius: 8, background: p.surfaceLo, border: `1px solid ${p.line}` }}>
    {options.map((o) => (
      <div key={o} onClick={() => onChange && onChange(o)} style={{
        padding: '5px 12px', fontSize: 12, fontWeight: 600,
        borderRadius: 6, cursor: 'pointer',
        color: o === value ? p.text : p.textMuted,
        background: o === value ? (p.name === 'dark' ? 'rgba(255,255,255,0.07)' : '#fff') : 'transparent',
...
const Stepper = ({ p, value }) => (
  <div style={{
    ...
    <div style={{ display: 'flex', flexDirection: 'column', marginLeft: 10 }}>
      <div style={{ padding: '0 4px', cursor: 'pointer' }}><SIcon.chevron size={10} style={{ transform: 'rotate(180deg)' }} /></div>
      <div style={{ padding: '0 4px', cursor: 'pointer' }}><SIcon.chevron size={10} /></div>
    </div>
  </div>
);
...
function EnvSwitcher({ p, open, onClose, env, onChange }) {
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        position: 'absolute', left: 20, bottom: 50, width: 320,
        background: p.surfaceHi, borderRadius: 14,
        border: `1px solid ${p.lineStrong}`,
        boxShadow: p.floatShadowLg,
        backdropFilter: 'blur(40px) saturate(180%)',
        overflow: 'hidden',
      }}>
        <div style={{ padding: '10px 14px', borderBottom: `1px solid ${p.line}`, fontSize: 11, fontWeight: 700, letterSpacing: 0.6, color: p.textDim, textTransform: 'uppercase' }}>
          Switch environment
        </div>
        {DATA.envs.map((e) => {
          const active = e.id === env.id;
          return (
            <div key={e.id} onClick={() => { onChange(e); onClose(); }} style={{
              padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10,
              cursor: 'pointer', background: active ? p.activeBg : 'transparent',
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 10.5, color: p.textDim, fontFamily: '"JetBrains Mono", monospace' }}>{e.host} · {e.vars} vars</div>
              </div>
              {active && <SIcon.check size={14} style={{ color: p.accent }} />}
            </div>
          );
        })}
        <div style={{ padding: '10px 14px', borderTop: `1px solid ${p.line}`, display: 'flex', alignItems: 'center', gap: 8, color: p.textMuted, fontSize: 12, cursor: 'pointer' }}>
