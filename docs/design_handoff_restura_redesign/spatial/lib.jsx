...
function makePalette(theme, accent) {
  if (theme === 'light') {
    return {
      ...
      activeBg: `${accent}1f`,
      floatShadow: '0 1px 0 rgba(255,255,255,0.6) inset, 0 8px 24px rgba(20,30,60,0.08), 0 1px 3px rgba(20,30,60,0.05)',
      floatShadowLg: '0 1px 0 rgba(255,255,255,0.6) inset, 0 20px 50px rgba(20,30,60,0.15), 0 4px 12px rgba(20,30,60,0.08)',
  ...
  return {
    ...
    activeBg: `${accent}26`,
    floatShadow: '0 1px 0 rgba(255,255,255,0.05) inset, 0 12px 36px rgba(0,0,0,0.55), 0 2px 6px rgba(0,0,0,0.4)',
    floatShadowLg: '0 1px 0 rgba(255,255,255,0.06) inset, 0 24px 60px rgba(0,0,0,0.65), 0 6px 18px rgba(0,0,0,0.5)',
...
const SIcon = {
  ...
  workflow:(p) => svg(p, "rect:3,3,6,6|rect:15,3,6,6|rect:9,15,6,6|M6 9v2a2 2 0 002 2h8a2 2 0 002-2V9"),
  ...
  cog:     (p) => svg(p, "circ:12,12,3|M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 11-2.83 2.83l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 11-4 0v-.09a1.65 1.65 0 00-1-1.51 1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 11-2.83-2.83l.06-.06a1.65 1.65 0 00.33-1.82 1.65 1.65 0 00-1.51-1H3a2 2 0 110-4h.09a1.65 1.65 0 001.51-1 1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 112.83-2.83l.06.06a1.65 1.65 0 001.82.33h.01a1.65 1.65 0 001-1.51V3a2 2 0 114 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 112.83 2.83l-.06.06a1.65 1.65 0 00-.33 1.82v.01a1.65 1.65 0 001.51 1H21a2 2 0 110 4h-.09a1.65 1.65 0 00-1.51 1z"),
  link:    (p) => svg(p, "M10 13a5 5 0 007.07 0l3-3a5 5 0 00-7.07-7.07L11.6 4.6|M14 11a5 5 0 00-7.07 0l-3 3a5 5 0 007.07 7.07L12.4 19.4"),
  ...
  star:    (p) => svg(p, "M12 2L15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2Z"),
  close:   (p) => svg(p, "M18 6L6 18M6 6l12 12"),
  play:    (p) => <svg width={p?.size||14} height={p?.size||14} viewBox="0 0 24 24" fill="currentColor" style={p?.style}><polygon points="6 4 20 12 6 20 6 4"/></svg>,
  pause:   (p) => svg(p, "rect:6,5,4,14|rect:14,5,4,14"),
  stop:    (p) => svg(p, "rect:5,5,14,14"),
  sparkle: (p) => svg(p, "M12 3v4M12 17v4M3 12h4M17 12h4M5.6 5.6l2.8 2.8M15.6 15.6l2.8 2.8M5.6 18.4l2.8-2.8M15.6 8.4l2.8-2.8"),
  command: (p) => svg(p, "M18 3a3 3 0 00-3 3v12a3 3 0 003 3 3 3 0 003-3 3 3 0 00-3-3H6a3 3 0 00-3 3 3 3 0 003 3 3 3 0 003-3V6a3 3 0 00-3-3 3 3 0 00-3 3 3 3 0 003 3h12a3 3 0 003-3 3 3 0 00-3-3z"),
  filter:  (p) => svg(p, "M22 3H2l8 9.46V19l4 2v-8.54L22 3z"),
  refresh: (p) => svg(p, "M23 4v6h-6|M1 20v-6h6|M3.51 9a9 9 0 0114.85-3.36L23 10|M1 14l4.64 4.36A9 9 0 0020.49 15"),
  trash:   (p) => svg(p, "M3 6h18|M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a2 2 0 012-2h4a2 2 0 012 2v2"),
  ...
  database:(p) => svg(p, "ellipse:12,5,9,3|M3 5v6c0 1.66 4 3 9 3s9-1.34 9-3V5|M3 11v6c0 1.66 4 3 9 3s9-1.34 9-3v-6"),
...
function MethodChip({ method, big, p }) {
  ...
  return (
    <span style={{
      ...
      fontSize: big ? 11.5 : 10, fontWeight: 700, letterSpacing: 0.4,
...
function Kbd({ children, p }) {
  return (
    <kbd style={{
      ...
      background: p.kbdBg, color: p.textMuted, border: `1px solid ${p.line}`,
      fontWeight: 500,
    }}>{children}</kbd>
  );
}
...
function StatusPill({ code, p }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 12, fontWeight: 700, color: c }}>{code} {label}</span>
    </div>
  );
}
...
function Stat({ label, value, p, accent }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
      <span style={{ fontSize: 10.5, color: p.textDim, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase' }}>{label}</span>
      <span style={{
        fontFamily: '"JetBrains Mono", monospace', fontSize: 12.5,
        color: accent || p.text, fontVariantNumeric: 'tabular-nums', fontWeight: 600,
      }}>{value}</span>
    </div>
  );
}
...
function hlJSON(src) {
  const C = { key: '#79b8ff', str: '#a5d6a7', num: '#ffab70', bool: '#c792ea', punct: '#94a3b8', var: '#f59e0b' };
  let s = src.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  s = s.replace(
    /("(?:\\.|[^"\\])*")(\s*:)?|\b(true|false|null)\b|-?\d+\.?\d*(?:[eE][-+]?\d+)?/g,
    (m, str, colon, kw) => {
      if (str) {
        if (colon) return `<span style="color:${C.key}">${str}</span><span style="color:${C.punct}">${colon}</span>`;
...
function hlGraphQL(src) {
  let s = src.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
  s = s.replace(/\b(query|mutation|subscription|fragment|on)\b/g, `<span style="color:#c792ea">$1</span>`);
...
const DATA = {
  collections: [
    { id: 'restura-api', name: 'Restura API', count: 14, open: true, children: [
    ...
    { id: 'realtime', name: 'Realtime', count: 4, open: true, children: [
    ...
    { id: 'agents', name: 'Agents & Tools', count: 5, children: [
      { id: 'gh-mcp', proto: 'MCP', method: 'MCP', name: 'gh-tools server' },
    ]},
  ...
  history: [
    { id: 'h1', method: 'GET',   status: 200, path: '/v2/users/42/orders',   ms: 114, ts: '2m ago', favorite: true },
    ...
    { id: 'h4', method: 'PATCH', status: 200, path: '/v2/orders/ord_7K9xQp2', ms: 188, ts: '22m ago' },
  ...
  envs: [
    { id: 'prod',    name: 'production',  color: '#22c55e', vars: 8, host: 'api.restura.dev', active: true },
...
Object.assign(window, {
  ACCENT_DEFAULT, makePalette, Floater, SIcon, MethodChip, ProtoChip, Kbd,
  StatusPill, Stat, hlJSON, hlGraphQL, DATA, METHOD_COL, PROTO_COL,
});
