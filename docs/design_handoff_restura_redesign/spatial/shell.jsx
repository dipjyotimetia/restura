...
const CONSOLE_LOGS = [
  {
    ...
    ts: '20:14:08.412',
    src: 'http',
    msg: 'GET https://api.restura.dev/v2/users/42/orders → 200 OK in 114ms',
  },
  {
    lvl: 'debug',
    ts: '20:14:08.418',
    src: 'env',
    msg: 'Resolved {{userId}} = 42 from environment "production"',
  },
  {
    lvl: 'debug',
    ts: '20:14:08.418',
    src: 'auth',
    msg: 'Attached Bearer token from vault (sk_live_••••••••)',
  },
  ...
  { lvl: 'warn', ts: '20:14:09.102', src: 'http', msg: 'X-RateLimit-Remaining low: 4982 / 10000' },
  {
    ...
    ts: '20:14:14.882',
    src: 'ws',
    msg: 'Schema mismatch: expected `status` to be string, got null on orders[1]',
  },
  {
    lvl: 'debug',
    ts: '20:14:14.882',
    src: 'history',
    msg: 'Saved response to history (req_4f3a91c8e2b6)',
  },
  ...
  return (
    ...
    >
      ...
      >
          ...
          {['error', 'warn', 'info', 'debug'].map((l) =>
            counts[l] ? (
              <span key={l} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <span
                  style={{ width: 6, height: 6, borderRadius: '50%', background: LVL_COL[l] }}
                />
                {counts[l]}
              </span>
            ) : null
        ...
        {!open && (
          <span
            style={{ fontSize: 11, color: p.textMuted, fontFamily: '"JetBrains Mono", monospace' }}
          >
            last: <span style={{ color: '#ef4444' }}>error</span> · 14m ago
          </span>
        )}
        {open && (
          <div
        ...
        >
          {logs.map((l, i) => (
            <div
              ...
              style={{
                display: 'grid',
                gridTemplateColumns: '110px 60px 70px 1fr',
                gap: 12,
                padding: '4px 14px',
                alignItems: 'baseline',
              }}
              ...
              <span
                ...
                dangerouslySetInnerHTML={{
                  __html: l.msg
                    ...
                    .replace(
                      /(\d+\s*ms\b)/g,
                      `<span style="color:${p.accent};font-weight:600">$1</span>`
                    )
                    .replace(
                      /\b(200 OK|201 Created)\b/g,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        <span
          style={{
            ...
            boxShadow: `0 0 6px ${env.color}`,
          }}
        />
        {env.name}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
        <SIcon.bolt size={11} /> 14 requests
      </div>
  ...
  const inner = (() => {
    switch (proto) {
      case 'HTTP':
        return <HttpView p={p} />;
      case 'GQL':
        return <GraphQLView p={p} />;
      case 'gRPC':
        return <GrpcView p={p} />;
      case 'WS':
        return <WebSocketView p={p} />;
      case 'SSE':
        return <SSEView p={p} />;
      case 'MCP':
        return <MCPView p={p} />;
      case 'Kafka':
        return <KafkaView p={p} />;
      default:
        return <HttpView p={p} />;
    }
  })();
