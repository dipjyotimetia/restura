    ...
    >
      {items.map((t) => {
        ...
        return (
          <div
            key={t.id || t.name}
            onClick={() => onChange && onChange(t.id || t.name)}
            style={{
              padding: '11px 12px',
              cursor: 'pointer',
              fontSize: 12.5,
...
function ParamRow({ p, row }) {
  return (
    <div
      style={{
        display: 'grid',
        gridTemplateColumns: '28px 1fr 1.5fr 1fr 22px',
        gap: 10,
        alignItems: 'center',
        padding: '8px 10px',
        borderRadius: 9,
    ...
    >
      ...
      <div style={{ color: p.textMuted, fontFamily: 'Geist, sans-serif', fontSize: 11.5 }}>
        {row.d}
      </div>
      <div style={{ color: p.textDim, textAlign: 'center', cursor: 'pointer' }}>
        <SIcon.close size={12} />
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', gap: 10 }}>
      ...
      >
          ...
          {url.split(/(\{\{[^}]+\}\})/).map((s, i) =>
            s.startsWith('{{') ? (
              ...
              <span
                ...
                style={{
                  color:
                    s.startsWith('http') || s.startsWith('ws')
                      ? p.textDim
                      : i % 2 === 0
                        ? p.text
                        : p.text,
        ...
        <div style={{ display: 'flex', gap: 2, paddingRight: 4 }}>
          <div style={{ padding: 6, color: p.textMuted, cursor: 'pointer' }}>
            <SIcon.copy size={13} />
          </div>
          <div style={{ padding: 6, color: p.textMuted, cursor: 'pointer' }}>
            <SIcon.history size={13} />
          </div>
          <div style={{ padding: 6, color: p.textMuted, cursor: 'pointer' }}>
            <SIcon.link size={13} />
          </div>
  ...
  return (
    ...
    >
      ...
      <SubTabBar
        ...
        right={
          <div style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '6px 0' }}>
            {['Pretty', 'Raw', 'Preview'].map((m) => (
              <div
                ...
                style={{
                  ...
                  color: m === view ? p.text : p.textMuted,
                  cursor: 'pointer',
                }}
              >
                {m}
            ...
            <div style={{ padding: 5, color: p.textMuted, cursor: 'pointer' }}>
              <SIcon.copy />
            </div>
            <div style={{ padding: 5, color: p.textMuted, cursor: 'pointer' }}>
              <SIcon.download />
          ...
          {DATA.headers.map(([k, v], i) => (
            <div
              ...
              style={{
                ...
                padding: '6px 0',
                borderBottom: i < DATA.headers.length - 1 ? `1px solid ${p.line}` : 'none',
                fontFamily: '"JetBrains Mono", monospace',
                fontSize: 12,
              }}
            >
        ...
        <div style={{ flex: 1, padding: 16, overflow: 'auto' }}>
          {DATA.timing.map((t, i) => (
            ...
            >
              ...
              >
                <div
                  style={{ width: `${(t.ms / 60) * 100}%`, height: '100%', background: t.color }}
                />
              </div>
              <span
                style={{
          ...
          >
            Total: <span style={{ color: p.text, fontWeight: 600 }}>110 ms</span> · Server-Timing:
            db;dur=42, cache;dur=2, compute;dur=14
          </div>
        </div>
      )}
      {(tab === 'Cookies' || tab === 'Tests') && (
        ...
        >
          {tab === 'Cookies'
            ? 'No cookies on this response.'
            : 'No tests defined. Add assertions in the Scripts tab.'}
        </div>
      )}
    </Floater>
  );
  ...
  return (
    <React.Fragment>
      <UrlBar
        p={p}
        method="GET"
        methodColor={METHOD_COL.GET}
        url="https://api.restura.dev/v2/users/{{userId}}/orders"
        methods
      />
...
function BodyEditor({ p }) {
  ...
  return (
    <React.Fragment>
      ...
      >
        {['none', 'JSON', 'form-data', 'x-www-form-urlencoded', 'GraphQL', 'raw', 'binary'].map(
          (t) => (
            <div
              key={t}
              onClick={() => setType(t)}
                ...
                background:
                  t === type
                    ? p.name === 'dark'
                      ...
                      : '#fff'
                    : 'transparent',
                color: t === type ? p.text : p.textMuted,
                cursor: 'pointer',
      ...
      <div
        style={{ display: 'flex', gap: 12, fontSize: 11, color: p.textMuted, padding: '4px 4px 0' }}
      >
        <span>JSON · 117 bytes</span>
        <span>·</span>
...
function AuthEditor({ p }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 14, padding: 4 }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 12.5, fontWeight: 600, color: p.text, marginBottom: 6 }}>
          Bearer token
        </div>
        <div style={{ fontSize: 11.5, color: p.textMuted, marginBottom: 12 }}>
          The token will be sent as the value of{' '}
          <code
            style={{
              background: p.surfaceLo,
              padding: '1px 5px',
        ...
        >
          ...
          >{`{{token}}`}</span>
          <span style={{ color: p.textDim, marginLeft: 8 }}>
            = <span style={{ color: '#22c55e' }}>sk_live_••••••••••</span>
          </span>
        </div>
      ...
      >
        {code.split('\n').map((line, i) => (
          <div key={i} style={{ display: 'flex' }}>
            <span style={{ width: 28, color: p.textDim, fontVariantNumeric: 'tabular-nums' }}>
              {i + 1}
            </span>
            <span
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: 8, maxWidth: 480 }}>
      {[
        ...
        {
          k: 'Encode URL automatically',
          desc: 'Percent-encode special characters in path',
          on: true,
        },
  ...
  const types = [
    {
      ...
      kind: 'OBJECT',
      color: '#a78bfa',
      fields: ['user(id): User', 'orders(...): OrderConn', 'me: User'],
    },
    {
      name: 'User',
      kind: 'OBJECT',
      color: '#4d9fff',
      fields: ['id: ID!', 'name: String', 'orders(...): OrderConn'],
    },
    {
      name: 'Order',
      kind: 'OBJECT',
      color: '#22c55e',
      fields: ['id: ID!', 'total: Float!', 'currency: Currency', 'status: OrderStatus'],
    },
    {
      name: 'OrderStatus',
      kind: 'ENUM',
      color: '#f59e0b',
      fields: ['PENDING', 'DELIVERED', 'IN_TRANSIT', 'CANCELLED'],
    },
  ];
  return (
    <React.Fragment>
      <UrlBar
        p={p}
        method="POST"
        methodColor={METHOD_COL.POST}
        url="https://api.restura.dev/graphql"
        sendLabel="Run"
      />
      <div
        style={{
          ...
          <SubTabBar
            ...
            right={
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingRight: 8 }}>
                <span style={{ fontSize: 10.5, color: p.textMuted, cursor: 'pointer' }}>
                  Prettify
                </span>
      ...
      >
            ...
            {services.map((s) => (
              <div key={s.name}>
                ...
                >
                  <SIcon.chevron
                    size={10}
                    style={{ opacity: 0.5, transform: s.open ? 'rotate(0)' : 'rotate(-90deg)' }}
                  />
                  <span
                    style={{
                      color: p.text,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, minHeight: 0 }}>
          <Floater p={p} radius={12} style={{ padding: 14 }}>
            ...
            >
              <span>
                <span style={{ color: p.textDim }}>in </span>
                <span style={{ color: '#a78bfa' }}>EchoRequest</span>
              </span>
              <span style={{ color: p.textDim }}>→</span>
              <span>
                <span style={{ color: p.textDim }}>out</span>{' '}
                <span style={{ color: '#a78bfa' }}>EchoReply</span>
              </span>
              <div style={{ flex: 1 }} />
              <span
                style={{
          ...
          >
            ...
            <div style={{ padding: '6px 14px 0', fontSize: 11, color: p.textMuted }}>
              ...
              <span style={{ color: '#f59e0b' }}>{`{{variable}}`}</span> for environment variables.
            </div>
            <pre
              style={{
                flex: 1,
        ...
        >
          ...
          <div style={{ borderTop: `1px solid ${p.line}`, padding: 14 }}>
            ...
            >
              <span style={{ color: p.textMuted }}>grpc-status</span>{' '}
              <span style={{ color: '#22c55e' }}>0 (OK)</span>
              <span style={{ color: p.textMuted }}>grpc-message</span>{' '}
              <span style={{ color: p.text }}>—</span>
              <span style={{ color: p.textMuted }}>x-server-trace</span>{' '}
              <span style={{ color: p.text }}>tr_4f3a91c8</span>
              <span style={{ color: p.textMuted }}>content-type</span>{' '}
              <span style={{ color: p.text }}>application/grpc</span>
            </div>
          </div>
          <div style={{ flex: 1 }} />
