    ...
    <Floater p={p} radius={14} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 6px 10px' }}>
        ...
        <div style={{
          ...
          alignItems: 'center', justifyContent: 'center', color: p.textMuted, cursor: 'pointer',
        }}><SIcon.more size={14} /></div>
      </div>
      ...
      {/* segmented tabs */}
      <div style={{
        display: 'flex', padding: 3, marginTop: 2,
        borderRadius: 9, background: p.surfaceLo,
      }}>
        {[
          ...
          <div key={t.k} onClick={() => setSidebarTab(t.k)} style={{
            flex: 1, padding: '5px 0', borderRadius: 6, textAlign: 'center',
            fontSize: 11, fontWeight: 600, cursor: 'pointer',
            color: t.k === sidebarTab ? p.text : p.textMuted,
            background: t.k === sidebarTab
              ? (p.name === 'dark' ? 'rgba(255,255,255,0.08)' : '#fff')
              : 'transparent',
            boxShadow: t.k === sidebarTab && p.name === 'light' ? '0 1px 3px rgba(0,0,0,0.06)' : 'none',
      ...
      {/* content */}
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 2, marginTop: 4 }}>
      ...
      {/* env footer */}
      <div onClick={onOpenEnv} style={{
        padding: '10px 10px', borderRadius: 10,
        background: p.surfaceLo, border: `1px solid ${p.line}`,
        display: 'flex', alignItems: 'center', gap: 9, cursor: 'pointer',
      }}>
        <span style={{
          width: 8, height: 8, borderRadius: '50%', background: env.color,
          boxShadow: `0 0 0 3px ${env.color}26`,
        }} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 11.5, fontWeight: 600, color: p.text }}>{env.name}</div>
          <div style={{ fontSize: 10, color: p.textDim, fontFamily: '"JetBrains Mono", monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{env.host}</div>
        </div>
  ...
  return (
    <React.Fragment>
      <div style={{
        ...
        <span style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.6, color: p.textDim, textTransform: 'uppercase' }}>
          5 collections
        </span>
        <div style={{ flex: 1 }} />
        <div style={{
          width: 22, height: 22, borderRadius: 6, color: p.textMuted, cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <React.Fragment key={c.id}>
          <div
            ...
            style={{
              ...
              fontSize: 12.5, fontWeight: 500, color: p.text,
              background: open[c.id] ? p.hoverBg : 'transparent',
              cursor: 'pointer',
            }}>
            <SIcon.chevron size={11} style={{
              transform: open[c.id] ? 'rotate(0deg)' : 'rotate(-90deg)',
              opacity: 0.5, transition: 'transform .12s',
            }} />
            <SIcon.folder size={13} style={{ color: open[c.id] ? p.accent : p.textMuted }} />
            <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</span>
            <span style={{ fontSize: 10.5, color: p.textDim, fontVariantNumeric: 'tabular-nums' }}>{c.count}</span>
          </div>
          {open[c.id] && c.children && c.children.map((ch) => (
            <div key={ch.id} onClick={() => onSelectRequest(ch)} style={{
              display: 'flex', alignItems: 'center', gap: 8,
              padding: '5px 10px 5px 28px', borderRadius: 7,
              fontSize: 12, color: ch.id === activeReqId ? p.text : p.textMuted,
              fontWeight: ch.id === activeReqId ? 600 : 500,
              background: ch.id === activeReqId ? p.activeBg : 'transparent',
              position: 'relative', cursor: 'pointer',
            }}>
              {ch.id === activeReqId && <div style={{
                position: 'absolute', left: 18, top: '50%', transform: 'translateY(-50%)',
                width: 3, height: 14, borderRadius: 2, background: p.accent,
...
function HistoryView({ p }) {
  ...
  return (
    <React.Fragment>
      <div style={{
        display: 'flex', gap: 4, padding: '2px 4px 6px',
        flexWrap: 'wrap',
      }}>
        {filters.map((f) => (
      ...
      {groups.map((g) => (
        <React.Fragment key={g.label}>
          <div style={{
            ...
            fontSize: 10, fontWeight: 700, letterSpacing: 0.6, color: p.textDim,
            ...
            return (
              <div key={h.id} style={{
                ...
                <span style={{
                  ...
                  background: ok ? 'rgba(34,197,94,0.16)' : h.status >= 500 ? 'rgba(239,68,68,0.18)' : 'rgba(245,158,11,0.16)',
                ...
                <span style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: 10.5, color: p.textDim, fontVariantNumeric: 'tabular-nums' }}>{h.ms}ms</span>
              </div>
            );
          })}
        </React.Fragment>
  ...
  return (
    <React.Fragment>
      <div style={{ padding: '4px 8px 6px', display: 'flex', alignItems: 'center' }}>
        <span style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: 0.6, color: p.textDim, textTransform: 'uppercase' }}>3 workflows</span>
        <div style={{ flex: 1 }} />
        <div style={{
          width: 22, height: 22, borderRadius: 6, color: p.textMuted, cursor: 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        return (
          <div key={w.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 12.5, fontWeight: 600, color: p.text, flex: 1 }}>{w.name}</span>
              <span style={{
                fontFamily: '"JetBrains Mono", monospace', fontSize: 9.5, fontWeight: 700, letterSpacing: 0.5,
                padding: '1px 5px', borderRadius: 4,
            ...
            <div style={{
              ...
              fontSize: 10.5, color: p.textMuted, fontFamily: '"JetBrains Mono", monospace',
            }}>
            ...
            <div style={{ display: 'flex', gap: 3, marginTop: 8 }}>
              {Array.from({ length: w.steps }).map((_, i) => (
                <div key={i} style={{
                  ...
                  background: ok
                    ? (i < w.steps - 1 ? '#22c55e' : '#22c55e')
                    : (i < 2 ? '#22c55e' : i === 2 ? '#ef4444' : p.line),
                }} />
              ))}
            </div>
          </div>
