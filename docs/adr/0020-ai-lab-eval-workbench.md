...
These guarantees do not broaden platform support: the AI Lab UI and in-app execution remain Electron-only while the CLI exposes only the headless subset above. Desktop supports saved Restura request tools, and both desktop and CLI bundles support deterministic fixture tools; MCP and sandbox providers remain unsupported (`web:false`, `desktop:false`) until wired end to end.
