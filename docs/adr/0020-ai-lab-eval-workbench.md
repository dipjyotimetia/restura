...
- **Headless CLI boundary.** `restura agent eval` currently registers only the OpenAI Responses adapter. It accepts environment-variable credentials and refuses desktop secret handles, suite base-URL overrides, judge graders, and every tool source until trusted CLI adapters exist. Exit `0` means every trial passed, exit `1` means an evaluated failed/error/cancelled report (including provider/runtime trial errors), and exit `2` means invalid input, unsupported configuration, or command/I/O failure. `--output` writes the complete local JSON trace report; CI artifact retention and access control remain the caller's responsibility.

These guarantees do not broaden platform support: the AI Lab UI and in-app execution remain Electron-only while the CLI exposes only the headless subset above. Saved Restura request tools remain the only shipped agent tool source, and agent MCP resolution and sandbox providers remain unsupported (`web:false`, `desktop:false`) until wired end to end.
