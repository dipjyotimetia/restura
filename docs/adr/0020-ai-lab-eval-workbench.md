...
- **Capability-driven providers.** Provider identifiers are open strings. Adapters publish capabilities instead of relying on model-name conditionals. Profiles cover OpenAI Responses and Chat, Anthropic Messages, Gemini GenerateContent, Azure OpenAI, Bedrock Converse, OpenRouter, Ollama, Hugging Face, and generic OpenAI-compatible gateways. The dedicated OpenAI Responses adapter supports tool calls, structured output, reasoning summaries, multimodal input, usage, and model discovery; its advertised server continuation capability is deliberately disabled. Other transports plug into `CallbackProviderAdapter`; the existing Electron AI Lab transport provides today’s shipped OpenAI/Anthropic/OpenRouter/Ollama/Hugging Face/compatible paths.
...
The capability matrix distinguishes fully wired features from extension contracts: desktop supports saved Restura HTTP tools and saved MCP profiles, while concrete sandbox providers remain marked unsupported until wired end-to-end.

## Addendum (2026-07-13): unified run guarantees

- **One hard-cancellable lifecycle per run surface.** Eval and agent execution each use a module-scoped run engine, so an active job survives an AI Lab tab switch and a second run on the same surface cannot start concurrently. An eval and an agent suite may still run at the same time. Renderer-generated operation IDs carry each `AbortSignal` through the validated Electron IPC boundary; cancellation also reaches queued/model calls, judge and calibration calls, and saved-request execution. Cancellation wins over a late success, so a cancelled job cannot publish a successful report.
- **Conservative, model-specific negotiation.** Discovery evidence is bound to the exact provider and model. Unknown models start from a conservative profile; opening the capability editor changes nothing. An override is enabled only after the user explicitly confirms that they are asserting the model's capabilities, is labelled user-asserted, and can be reset to discovered defaults. Unknown pricing remains unknown rather than becoming `$0`.
- **Run-wide budgets and task-aware grading.** `maxTokens` limits total input plus output tokens across the entire agent trial, with the exact limit allowed. Missing, partial, negative, non-integer, or non-finite usage fails closed when a token budget must be enforced. Exact/contains graders use the task reference when no grader value is supplied. Judge and calibration calls share cancellation and resource accounting; judge panels enforce distinct provider/model identities, quorum, agreement, optional calibration, output-token bounds, and optional cost bounds, and fail closed on ties, insufficient quorum, unknown required cost, or failed calibration.
- **Stateless OpenAI Responses replay.** The CLI adapter performs stateless encrypted reasoning and function-call replay using `store: false`; server-side `previous_response_id` continuation is disabled. Encrypted provider output items and function-call/output messages are replayed in the next request instead of retaining server state.
- **Durable, bounded desktop reports.** Completed agent reports are sanitized before display, persistence, and export. Sensitive keys, headers, URL query values, and recognized secret-shaped body values are redacted; large content is marked as truncated. Opaque sensitive data in model or tool output may not be recognized, so exports still require appropriate access controls. The awaited report repository surfaces write failures so the UI can retain the sanitized in-memory report and offer retry. Agent reports are limited to 2 MiB each, the newest 20 reports, and 20 MiB total, with deterministic oldest-first eviction. Persisted suites/reports are schema-validated during hydration and migration; invalid entries are quarantined individually instead of resetting unrelated AI Lab state.
- **Headless CLI boundary.** `restura agent eval` currently registers only the OpenAI Responses adapter. It accepts environment-variable credentials and refuses desktop secret handles, suite base-URL overrides, judge graders, and every live or remote tool source until trusted CLI adapters exist. Git-native bundles may register deterministic read-only fixture tools, and their baseline gates fail a regressed report. Exit `0` means every trial passed, exit `1` means an evaluated failed/error/cancelled report (including provider/runtime trial errors), and exit `2` means invalid input, unsupported configuration, or command/I/O failure. `--output` writes the complete local JSON trace report; CI artifact retention and access control remain the caller's responsibility.

These guarantees do not broaden platform support: the AI Lab UI and in-app execution remain Electron-only while the CLI exposes only the headless subset above. Desktop supports saved Restura request tools, deterministic fixture tools, and saved MCP profiles with fresh policy-enforced sessions, allowlists, cancellation cleanup, and per-call approval. Sandboxes remain unsupported (`web:false`, `desktop:false`) until a concrete provider is wired end to end.
