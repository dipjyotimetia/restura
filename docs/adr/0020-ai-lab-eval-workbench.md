...
- **Durable, bounded desktop reports.** Completed agent reports are sanitized before display, persistence, and export. Sensitive keys, headers, URL query values, and body secrets are redacted; large content is marked as truncated. The awaited report repository surfaces write failures so the UI can retain the sanitized in-memory report and offer retry. Agent reports are limited to 2 MiB each, the newest 20 reports, and 20 MiB total, with deterministic oldest-first eviction. Invalid persisted suites/reports are quarantined individually instead of resetting unrelated AI Lab state.
- **Headless CLI boundary.** `restura agent eval` currently registers only the OpenAI Responses adapter. It accepts environment-variable credentials and refuses desktop secret handles, suite base-URL overrides, judge graders, and every tool source until trusted CLI adapters exist. Exit `0` means every trial passed, exit `1` means an evaluated failed/error/cancelled report, and exit `2` means invalid input or runtime setup. `--output` writes the complete local JSON trace report; CI artifact retention and access control remain the caller's responsibility.

These guarantees do not broaden platform support: AI Lab authoring and desktop execution remain Electron-only, saved Restura request tools remain the only shipped agent tool source, and agent MCP resolution and sandbox providers remain unsupported (`web:false`, `desktop:false`) until wired end to end.
