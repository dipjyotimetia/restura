...
All streaming handlers in `electron/main/` (`grpc-handler.ts`, `grpc-reflection-handler.ts`, `mcp-handler.ts`, `sse-handler.ts`, `socketio-handler.ts`, `websocket-handler.ts`, `kafka-handler.ts`) call `assertUrlHostnameSafe` (or a transport-specific equivalent for Kafka — see below) before initiating the transport-level connect, and tear down on renderer destruction. They share the owner-scoped `bindRendererCleanup` + `disposeByOwner` lifecycle helpers.

**Kafka metadata discovery: pre-flight only with no public `lookup` hook**

`@platformatic/kafka` 2.8 is a JavaScript Kafka protocol client built on Node networking, not a C++ binding. Its public client options do not expose a DNS `lookup` injection point, and Kafka metadata can introduce brokers beyond the bootstrap list. The handler therefore calls `assertKafkaBrokersSafe` before constructing the client; it allows private broker clusters while rejecting cloud-metadata literals and blocked hostnames. Metadata-discovered broker addresses cannot currently be pinned or revalidated through the dependency's public API.
...
**Residual rebind window:** Kafka's pre-flight resolves once; a TTL=0 DNS swap before the dependency connects is not pinned, and metadata-discovered broker addresses bypass `assertKafkaBrokersSafe`. This remains an accepted limitation until the dependency exposes a supported resolver/connection hook.
