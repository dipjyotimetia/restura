...
SSRF guards on the Worker, the Node/Docker server (`shared/protocol/url-validation.ts`), and the Electron path block RFC 1918, CGNAT (100.64.0.0/10), link-local (169.254.0.0/16), cloud metadata endpoints, IPv6 unique-local + link-local, and IPv4-mapped IPv6 addresses. **Direct HTTP, gRPC, WebSocket, and SSE connections additionally pin the connect to the validated address** (closing the DNS-rebind window): HTTP via undici's `Agent.connect.lookup` (`createSecureLookup`); WebSocket/SSE via `createPinnedLookup` / `createPinnedFetch` (`electron/main/security/safe-connect.ts`); gRPC via `connect-node`'s `nodeOptions.lookup` (ADR 0022 replaced the old grpc-js IP-literal dial). The remaining direct transports (Socket.IO, MCP, gRPC reflection, Kafka, MQTT) use the **pre-flight** guard in `electron/main/security/dns-guard.ts`: `assertUrlHostnameSafe(url, ...)` resolves the hostname and applies `assertResolvedAddressAllowed` to every record before connect, but cannot pin — a TTL=0 rebind between resolve and connect is not mitigated for them. Managed proxy routes validate schemes, hostnames, literal IPs, settings, and metadata targets locally, then deliberately delegate hostname resolution and resolved-address enforcement to the administrator-controlled proxy. Kafka and MQTT broker addresses additionally go through `kafka-broker-guard.ts` / `mqtt-broker-guard.ts`. Tracked as future work in ADR-0006.
